...
function createNotificationBadge() {
  ...
  // Trouver le bouton de déconnexion ou le dernier élément
  const logoutBtn = document.querySelector('.navbar-btn');
  ...
  if (logoutBtn) {
    logoutBtn.parentNode.insertBefore(notifBell, logoutBtn);
  } else {
...
function displayNotifications(notifications) {
  ...
  list.innerHTML = notifications.map(notif => `
    <div class="notification-item ${notif.read ? '' : 'unread'}" onclick="handleNotificationClick('${notif.id}', '${notif.action_url || ''}', ${notif.read})">
      ...
      <div class="notification-footer">
        ...
        <div style="display: flex; gap: 8px;">
          ${notif.action_url && notif.action_label ? `
            <button class="notification-btn" onclick="event.stopPropagation(); handleNotificationAction('${notif.id}', '${notif.action_url}', ${JSON.stringify(notif.data).replace(/"/g, '&quot;')})">
              ${notif.action_label}
            </button>
          ` : ''}
...
async function markNotificationAsRead(notifId) {
  try {
    await authenticatedFetch(`/notifications/${notifId}/read`, {
      method: 'PUT'
    });
    await loadNotifications();
  } catch (error) {
